...
class EventConfig:
    ...
    def __addToMap(self, e_id):
        event_type = (
            None
            if not self.__cfg.has_option(e_id, "event_type")
            else self.__cfg.getint(e_id, "event_type")
        )
        if (
            self.__cfg.has_option(e_id, "event_title")
            and self.__cfg.has_option(e_id, "start_date")
            and self.__cfg.has_option(e_id, "end_date")
            and self.__cfg.has_option(e_id, "data_file")
        ):
            self.events[e_id] = {
